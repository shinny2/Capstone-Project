...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                html.Br(),
                                dcc.Dropdown(id='site-dropdown',options=[{'label': 'All Sites', 'value': 'ALL'},{'label': 'CCAFS LC-40', 'value': 'CCAFS LC-40'}, {'label': 'VAFB SLC-4E', 'value': 'VAFB SLC-4E'}, {'label':'KSC LC-39A', 'value': 'KSC LC-39A'}, {'label':'CCAFS SLC-40', 'value':'CCAFS SLC-40'}],value='ALL',placeholder="All Sites",searchable=True),
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        data1 = spacex_df[spacex_df['Launch Site'] == entered_site].groupby(['Launch Site', 'class']).size().reset_index(name='class count') 
        fig = px.pie(data1, values='class count', names='class', title = 'Total Success Launches for site ' + entered_site)
...
def get_scatter_chart(site_dropdown, payload_range):
    ...
    if(site_dropdown=='ALL'):
        ...
        filtered1 = filtered[filtered['Launch Site']==site_dropdown]
        fig = px.scatter(filtered1,x = 'Payload Mass (kg)', y = 'class',color = 'Booster Version Category')
        return fig
